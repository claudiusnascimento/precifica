import type { ThemePreset } from '@/types/theme';

/**
 * Preset shadcn `bIm515k` (style: vega, base: neutral, theme: blue).
 * Valores HSL mapeados a partir do gerador oficial do shadcn/ui.
 */
export const customOrangeTheme: ThemePreset = {
    id: 'custom-orange',
    label: 'Custom Orange',
    shadcnCode: 'bIm515k',
    variables: {
        light: {
            '--radius': '0.625rem',
            '--background': 'hsl(0 0% 100%)',
            '--foreground': 'hsl(0 0% 3.9%)',
            '--card': 'hsl(0 0% 100%)',
            '--card-foreground': 'hsl(0 0% 3.9%)',
            '--popover': 'hsl(0 0% 100%)',
            '--popover-foreground': 'hsl(0 0% 3.9%)',
            '--primary': 'hsl(221.2 83.2% 53.3%)',
            '--primary-foreground': 'hsl(210 40% 98%)',
            '--secondary': 'hsl(0 0% 96.1%)',
            '--secondary-foreground': 'hsl(0 0% 9%)',
            '--muted': 'hsl(0 0% 96.1%)',
            '--muted-foreground': 'hsl(0 0% 45.1%)',
            '--accent': 'hsl(0 0% 96.1%)',
            '--accent-foreground': 'hsl(0 0% 9%)',
            '--destructive': 'hsl(0 84.2% 60.2%)',
            '--border': 'hsl(0 0% 89.8%)',
            '--input': 'hsl(0 0% 89.8%)',
            '--ring': 'hsl(221.2 83.2% 53.3%)',
            '--chart-1': 'hsl(221.2 83.2% 53.3%)',
            '--chart-2': 'hsl(212 95% 68%)',
            '--chart-3': 'hsl(216 92% 60%)',
            '--chart-4': 'hsl(210 98% 78%)',
            '--chart-5': 'hsl(212 97% 87%)',
            '--sidebar': 'hsl(0 0% 98%)',
            '--sidebar-foreground': 'hsl(0 0% 3.9%)',
            '--sidebar-primary': 'hsl(221.2 83.2% 53.3%)',
            '--sidebar-primary-foreground': 'hsl(210 40% 98%)',
            '--sidebar-accent': 'hsl(0 0% 96.1%)',
            '--sidebar-accent-foreground': 'hsl(0 0% 9%)',
            '--sidebar-border': 'hsl(0 0% 89.8%)',
            '--sidebar-ring': 'hsl(221.2 83.2% 53.3%)',
        },
        dark: {
            '--radius': '0.625rem',
            '--background': 'hsl(0 0% 3.9%)',
            '--foreground': 'hsl(0 0% 98%)',
            '--card': 'hsl(0 0% 3.9%)',
            '--card-foreground': 'hsl(0 0% 98%)',
            '--popover': 'hsl(0 0% 3.9%)',
            '--popover-foreground': 'hsl(0 0% 98%)',
            '--primary': 'hsl(217.2 91.2% 59.8%)',
            '--primary-foreground': 'hsl(222.2 47.4% 11.2%)',
            '--secondary': 'hsl(0 0% 14.9%)',
            '--secondary-foreground': 'hsl(0 0% 98%)',
            '--muted': 'hsl(0 0% 14.9%)',
            '--muted-foreground': 'hsl(0 0% 63.9%)',
            '--accent': 'hsl(0 0% 14.9%)',
            '--accent-foreground': 'hsl(0 0% 98%)',
            '--destructive': 'hsl(0 62.8% 30.6%)',
            '--border': 'hsl(0 0% 14.9%)',
            '--input': 'hsl(0 0% 14.9%)',
            '--ring': 'hsl(224.3 76.3% 48%)',
            '--chart-1': 'hsl(221.2 83.2% 53.3%)',
            '--chart-2': 'hsl(212 95% 68%)',
            '--chart-3': 'hsl(216 92% 60%)',
            '--chart-4': 'hsl(210 98% 78%)',
            '--chart-5': 'hsl(212 97% 87%)',
            '--sidebar': 'hsl(0 0% 9%)',
            '--sidebar-foreground': 'hsl(0 0% 98%)',
            '--sidebar-primary': 'hsl(224.3 76.3% 48%)',
            '--sidebar-primary-foreground': 'hsl(0 0% 100%)',
            '--sidebar-accent': 'hsl(0 0% 14.9%)',
            '--sidebar-accent-foreground': 'hsl(0 0% 98%)',
            '--sidebar-border': 'hsl(0 0% 14.9%)',
            '--sidebar-ring': 'hsl(224.3 76.3% 48%)',
        },
    },
};
